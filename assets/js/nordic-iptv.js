document.querySelectorAll('.pfqq').forEach(function(btn){btn.addEventListener('click',function(){var item=btn.closest('.pfqi'),open=item.classList.contains('open');document.querySelectorAll('.pfqi.open').forEach(function(x){x.classList.remove('open')});if(!open)item.classList.add('open');});});
